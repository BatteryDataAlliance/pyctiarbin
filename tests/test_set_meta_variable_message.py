...
def test_set_meta_variable_client_msg():
    ...
    packed_msg = Msg.SetMetaVariable.Client.pack(msg_dict)
    assert (packed_msg == msg_bin)

    # Checking parsing the example message binary and that it matches the msg_dict
    parsed_msg = Msg.SetMetaVariable.Client.unpack(msg_bin)
...
def test_set_meta_variable_server_msg():
    ...
    parsed_msg = Msg.SetMetaVariable.Server.unpack(msg_bin)
    assert (parsed_msg == msg_dict)
    ...
    packed_msg = Msg.SetMetaVariable.Server.pack(buildable_msg_dict)
    parsed_msg = Msg.SetMetaVariable.Server.unpack(packed_msg)
    assert (parsed_msg == msg_dict)